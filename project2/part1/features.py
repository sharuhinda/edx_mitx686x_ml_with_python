...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    #X_centered, _ = center_data(X) # use ready-to-use function defined below
    X_centered = X - feature_means # self-written centering, feature_means must have shape (d,)
    ...
    # X_tilda * V, V comes from V_full (result of principal_components() function with shape (d x k))
    # X_tilda has shape of (n x d)
    # V is part of V_full with shape (d x n_components)
    # Thus X_tilda * V will have shape of (n x n_components)
