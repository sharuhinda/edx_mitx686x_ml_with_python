...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.rand() < epsilon:
        ...
        action_index = q_func[state_1, state_2].max(axis=1).argmax() # row where max value of whole matrix
        object_index = q_func[state_1, state_2].max(axis=0).argmax() # col with max value of whole matrix
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    Args:
        q_func (np.ndarray): current Q-function
        current_state_1, current_state_2 (int, int): two indices describing the current state <= seem to be (room, quest) indices which describe hidden state of the game
        action_index (int): index of the current action <= this and next rows form command
        object_index (int): index of the current object
        reward (float): the immediate reward the agent receieves from playing current command
        next_state_1, next_state_2 (int, int): two indices describing the next state <= (room, quest) for next state
        terminal (bool): True if this episode is over
    Note: GAMMA and ALPHA are available
    ...
    if terminal:
        q_func[current_state_1, current_state_2, action_index,
           object_index] = (1. - ALPHA) * q_func[current_state_1, current_state_2, action_index, object_index] + ALPHA * reward
    else:
        q_func[current_state_1, current_state_2, action_index,
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        # TODO Your code here
        action_index, object_index = epsilon_greedy(dict_room_desc[current_room_desc], dict_quest_desc[current_quest_desc], q_func, epsilon=epsilon)
