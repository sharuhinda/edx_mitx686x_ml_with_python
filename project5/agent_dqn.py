...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.rand() < epsilon:
        ...
        action_index, object_index = torch.argmax(q_values_action).item(), torch.argmax(q_values_object).item()
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_current = 1/2 * (q_values_action_current[action_index] + q_values_object_current[object_index])

    # TODO Your code here
    if not terminal:
        y = (1. - ALPHA) * q_current + ALPHA * (reward + GAMMA * maxq_next)
    else:
        y = (1. - ALPHA) * q_current + ALPHA * reward

    #loss = None
    loss = (y - q_current)**2 / 2
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        # TODO Your code here
        action_index, object_index = epsilon_greedy(state_vector=current_state_vector, epsilon=epsilon)
